...
public class UserController {
  ...
  public ResponseEntity<UserInfoDto> getMyUserInfo(@RequestHeader("Authorization") String token) {
    try {
      ...
      return ResponseEntity.status(HttpStatus.OK)
          .body(userInfoDto); // Trả về thông tin của user và mã trạng thái 200 OK
    } catch (Exception e) {
      log.error("Failed to get user info", e);
      return ResponseEntity.status(HttpStatus.BAD_REQUEST)
          .body(null); // Trả về mã trạng thái 400 Bad Request
    }
  }
  ...
  // Update the user
  @PutMapping("/update")
  public ResponseEntity<String> updateUser(@Valid @ModelAttribute UpdateUserDto updateUserDto,
